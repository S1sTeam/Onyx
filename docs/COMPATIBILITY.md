...
54. Login protocol lock is available via `login-protocol-lock-enabled` + `login-protocol-lock-version` (for finished-v1 production lock to protocol `774`).
55. Finished-v1 production profile can be applied via `scripts/configure-finished-v1.ps1` (protocol lock + stable play/proxy defaults; optional `-DisableLoginProtocolLock` for matrix benchmark compatibility runs).
56. Finished-v1 end-to-end gate is available via `scripts/e2e-finished-v1.ps1`.
57. Duration-based soak loops are available via `scripts/e2e-play-soak-duration.ps1` (e.g. `-Hours 24`).
58. Runtime console operations include metrics/help commands (`OnyxServer`: `help, metrics, players, save`; `OnyxProxy`: `help, metrics`).
