...
$ExpectedUsername = "ChatBridgeProbe"
$ExpectedChatCommand = "/ping"
$ExpectedDirectCommand = "echo bridge"
$ExpectedResponseChatCommand = "Onyx pong"
$ExpectedResponseDirectCommand = "Onyx echo: bridge"
$SpawnX = 2.5
$SpawnY = 70.0
$SpawnZ = -3.0
...
try {
    ...
    Write-McString $packetBody $ExpectedChatCommand
    }
    ...
    Write-McString $packetBody $ExpectedDirectCommand
    }
    $stream.Write($commandPacket, 0, $commandPacket.Length)
    $sentChat = $true
    $sentCommand = $true
    $receivedChatCommandResponse = $false
    $receivedDirectCommandResponse = $false
    $disconnectReason = ""
    $maxPackets = 128
    ...
    if ($responseText -eq $ExpectedResponseChatCommand) {
    
    
    
    $receivedChatCommandResponse = $true
    
    
    }
    
    
    if ($responseText -eq $ExpectedResponseDirectCommand) {
    
    
    
    $receivedDirectCommandResponse = $true
    ...
    throw "Server did not record total command packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-chat-command-packets=1') {
    
    throw "Server did not record bridged chat command count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-direct-command-packets=1') {
    
    throw "Server did not record direct command count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-command-ping=1') {
    
    throw "Server did not record ping command dispatch count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-command-echo=1') {
    
    throw "Server did not record echo command dispatch count: $disconnectReason"
    }
    ...
    if ($disconnectReason -notmatch 'input-last-command=echo bridge') {
    
    throw "Server did not report expected last command: $disconnectReason"
    }
    if (-not $receivedChatCommandResponse) {
    
    throw "Did not receive expected bridged chat command response packet"
    }
    if (-not $receivedDirectCommandResponse) {
    
    throw "Did not receive expected direct command response packet"
    }
    Write-Host "[ONYX] E2E_PLAY_CHAT_COMMAND_BRIDGE_OK"
