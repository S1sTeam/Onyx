...
$ExpectedUsername = "PersistIdleProbe"
$ObserveDurationMs = 3200
...
try {
    ...
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-session-max-packets\s*=.*$' -line "play-session-max-packets = 4"
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-session-idle-timeout-ms\s*=.*$' -line "play-session-idle-timeout-ms = 250"
    ...
    throw "Expected idle-timeout disconnect, but connection stayed open"
    }
    if ($disconnectReason -notmatch 'terminal=idle-timeout') {
    
    throw "Disconnect reason does not indicate idle-timeout: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'persistent=true') {
    
    throw "Disconnect reason does not indicate persistent mode: $disconnectReason"
    }
    Write-Host "[ONYX] E2E_PLAY_PERSISTENT_IDLE_TIMEOUT_OK"
