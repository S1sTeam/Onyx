...
$ChatPacketId = 0x73
$ExpectedUsername = "InputRateProbe"
$ChatOne = "spam_1"
$ChatTwo = "spam_2"
$ChatThree = "spam_3"
$SpawnX = 2.5
...
try {
    ...
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-session-duration-ms\s*=.*$' -line "play-session-duration-ms = 5000"
    ...
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-input-rate-limit-window-ms\s*=.*$' -line "play-input-rate-limit-window-ms = 2000"
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-input-rate-limit-max-packets\s*=.*$' -line "play-input-rate-limit-max-packets = 2"
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-input-rate-limit-max-chat-packets\s*=.*$' -line "play-input-rate-limit-max-chat-packets = 10"
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-input-rate-limit-max-command-packets\s*=.*$' -line "play-input-rate-limit-max-command-packets = 10"
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-input-response-enabled\s*=.*$' -line "play-input-response-enabled = false"
    ...
    $chatPacketOne = Build-Packet {
    
    param($packetBody)
    
    Write-VarInt $packetBody $ChatPacketId
    
    Write-McString $packetBody $ChatOne
    }
    $stream.Write($chatPacketOne, 0, $chatPacketOne.Length)
    $chatPacketTwo = Build-Packet {
    
    param($packetBody)
    
    Write-VarInt $packetBody $ChatPacketId
    
    Write-McString $packetBody $ChatTwo
    }
    $stream.Write($chatPacketTwo, 0, $chatPacketTwo.Length)
    $chatPacketThree = Build-Packet {
    
    param($packetBody)
    
    Write-VarInt $packetBody $ChatPacketId
    
    Write-McString $packetBody $ChatThree
    }
    $stream.Write($chatPacketThree, 0, $chatPacketThree.Length)
    $sentChatPackets = 3
    $disconnectReason = ""
    $maxPackets = 128
    ...
    if ($sentChatPackets -ne 3) {
    
    throw "Chat packets were not sent"
    }
    if ([string]::IsNullOrWhiteSpace($disconnectReason)) {
    
    throw "Did not receive final disconnect packet"
    }
    if ($disconnectReason -notmatch 'terminal=input-rate-limit-packets') {
    
    throw "Server did not terminate with input rate limit packet trigger: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-packets=3') {
    
    throw "Server did not report input packet total: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-chat-packets=3') {
    
    throw "Server did not record chat packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-command-packets=0') {
    
    throw "Server did not record command packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-rate-limit-hits=1') {
    
    throw "Server did not record input rate-limit hit count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-rate-limit-reason=packets') {
    
    throw "Server did not report input rate-limit reason: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-rate-window-packets=3') {
    
    throw "Server did not report input rate window packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-last-chat=spam_3') {
    
    throw "Server did not report expected last chat: $disconnectReason"
    }
    Write-Host "[ONYX] E2E_PLAY_INPUT_RATE_LIMIT_OK"
