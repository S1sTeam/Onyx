param(
    ...
    [int]$ProxyPort = 30765,
    [int]$BackendPort = 30766 )
Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"
$Protocol = 47
$DisconnectPacketId = 0x40
$KeepAliveClientboundId = 0x5A
$KeepAliveServerboundId = 0x5B
...
try {
    ...
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-keepalive-ack-timeout-ms\s*=.*$' -line "play-keepalive-ack-timeout-ms = 220"
    ...
    Write-UShortBE $packetBody $ProxyPort
    ...
    Write-McString $packetBody "KeepAliveMvp"
    }
    ...
    if ($disconnectReason -notmatch 'terminal=keepalive-ack-timeout') {
    
    throw "Disconnect reason does not include keepalive timeout terminal marker: $disconnectReason"
    }
    ...
    if ($disconnectReason -notmatch 'keepalive-ack-timeouts=1') {
    
    throw "Disconnect reason does not include keepalive ack timeout metric: $disconnectReason"
    }
    ...
    if ($ackedByServer -ne 0) {
    
    throw "Server reported unexpected keepalive acks in timeout scenario: $disconnectReason"
    }
    Write-Host "[ONYX] E2E_PLAY_KEEPALIVE_TIMEOUT_OK"
    Write-Host "[ONYX] KEEPALIVE_CLIENT_RECEIVED=$receivedKeepalive"
