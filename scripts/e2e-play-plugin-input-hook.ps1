...
$ExpectedUsername = "PluginHookProbe"
$ExpectedChat = "filter_me"
$ExpectedCommand = "ping"
$ExpectedResponseChat = "Onyx plugin chat override"
$ExpectedResponseCommand = "Onyx plugin command override"
$UnexpectedResponseChat = "Onyx chat: filter_me"
$UnexpectedResponseCommand = "Onyx pong"
$SpawnX = 2.5
$SpawnY = 70.0
$SpawnZ = -3.0
...
function Resolve-JdkTool([string]$toolName) {
    ...
    return $toolName}
function Build-E2EInputHookPlugin([string]$serverJarPath, [string]$pluginsDir) {
    $javac = Resolve-JdkTool "javac"
    $workDir = Join-Path $pluginsDir "_e2e_input_hook_build"
    ...
    $sourcePath = Join-Path $srcDir "E2EInputHookPlugin.java"
    $servicePath = Join-Path $servicesDir "dev.onyx.server.plugin.OnyxServerPlugin"
    $pluginJarPath = Join-Path $pluginsDir "e2e-input-hook-plugin.jar"
    $source = @'
...
public final class E2EInputHookPlugin implements OnyxServerPlugin {
    @Override
    public String id() {
        return "e2e-input-hook";
    }

    @Override
    public void onEnable(OnyxServerContext context) {
        context.logger().info("E2E input hook plugin enabled");
    }

    @Override
    public OnyxServerInputResult onInputChat(OnyxServerInput input) {
        if ("filter_me".equalsIgnoreCase(input.message())) {
            return OnyxServerInputResult.consume("Onyx plugin chat override");
        }
        return OnyxServerInputResult.pass();
    }

    @Override
    public OnyxServerInputResult onInputCommand(OnyxServerInput input) {
        if ("ping".equalsIgnoreCase(input.message().trim())) {
            return OnyxServerInputResult.consume("Onyx plugin command override");
        }
        return OnyxServerInputResult.pass();
    }
}
'@
    ...
    [System.IO.File]::WriteAllText($servicePath, "dev.onyx.e2e.E2EInputHookPlugin", $utf8NoBom)
    & $javac --release 21 -cp $serverJarPath -d $classesDir $sourcePath
    if ($LASTEXITCODE -ne 0) {
    
    throw "javac failed while building e2e input hook plugin"
    }
    ...
    throw "failed to pack e2e input hook plugin jar"
    }
    ...
    if ($entryNames -notcontains "dev/onyx/e2e/E2EInputHookPlugin.class") {
...
try {
    ...
    $pluginArtifact = Build-E2EInputHookPlugin -serverJarPath $serverJar -pluginsDir $serverPluginsDir
    ...
    $commandPacket = Build-Packet {
    
    param($packetBody)
    
    Write-VarInt $packetBody $CommandPacketId
    
    Write-McString $packetBody $ExpectedCommand
    }
    ...
    $receivedUnexpectedChatResponse = $false
    $receivedUnexpectedCommandResponse = $false
    $disconnectReason = ""
    $maxPackets = 128
    ...
    if ($responseText -eq $ExpectedResponseChat) {
    ...
    if ($responseText -eq $ExpectedResponseCommand) {
    ...
    if ($responseText -eq $UnexpectedResponseChat) {
    
    
    
    $receivedUnexpectedChatResponse = $true
    
    
    }
    
    
    if ($responseText -eq $UnexpectedResponseCommand) {
    
    
    
    $receivedUnexpectedCommandResponse = $true
    ...
    throw "Chat packet was not sent"
    }
    if (-not $sentCommand) {
    
    throw "Command packet was not sent"
    }
    ...
    if ($disconnectReason -notmatch 'input-chat-packets=1') {
    
    throw "Server did not record chat packet: $disconnectReason"
    }
    ...
    if ($disconnectReason -notmatch 'input-direct-command-packets=1') {
    
    throw "Server did not report direct-command packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-plugin-chat-handled=1') {
    
    throw "Server did not report plugin chat handled count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-plugin-command-handled=1') {
    
    throw "Server did not report plugin command handled count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-command-ping=0') {
    
    throw "Built-in ping counter should remain zero when plugin handles command: $disconnectReason"
    }
    ...
    if ($disconnectReason -notmatch 'input-plugin-response-packets=2') {
    
    throw "Server did not record plugin response packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'input-last-chat=filter_me') {
    ...
    throw "Did not receive expected chat response packet"
    }
    if (-not $receivedCommandResponse) {
    
    throw "Did not receive expected command response packet"
    }
    if ($receivedUnexpectedChatResponse) {
    
    throw "Received unexpected built-in chat response packet"
    }
    if ($receivedUnexpectedCommandResponse) {
    
    throw "Received unexpected built-in command response packet"
    }
    $testPassed = $true
    Write-Host "[ONYX] E2E_PLAY_PLUGIN_INPUT_HOOK_OK"
