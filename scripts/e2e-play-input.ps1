...
try {
    ...
    if (($packet.Id -eq $BootstrapInitPacketId -or $packet.Id -eq $BootstrapSpawnPacketId) -and -not $sentChat) {
    ...
    if ($disconnectReason -notmatch 'input-last-command=hub') {
    
    throw "Server did not report expected last command: $disconnectReason"
    }
    Write-Host "[ONYX] E2E_PLAY_INPUT_OK"
