...
$ExpectedUsername = "StatusOnProbe"
...
function Upsert-Line([string]$content, [string]$pattern, [string]$line) {
    ...
    return $content + $line}
function Extract-OnlineCount([string]$json) {
    if ($json -match '"online"\s*:\s*(\d+)') {
    
    return [int]$matches[1]
    }
    throw "Could not parse online count from status json: $json"}
...
try {
    ...
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-session-idle-timeout-ms\s*=.*$' -line "play-session-idle-timeout-ms = 0"
    ...
    $loginClient = New-Object System.Net.Sockets.TcpClient
    $loginClient.Connect("127.0.0.1", $BackendPort)
    $loginStream = $loginClient.GetStream()
    $loginStream.ReadTimeout = 8000
    ...
    $loginStream.Write($handshake, 0, $handshake.Length)
    $loginStream.Write($loginStart, 0, $loginStart.Length)
    $loginSuccess = Read-Packet $loginStream
    ...
    $loginStream.Write($loginAck, 0, $loginAck.Length)
    $configFinish = Read-Packet $loginStream
    ...
    $loginStream.Write($clientFinish, 0, $clientFinish.Length)
    Start-Sleep -Milliseconds 150
    $statusClient = New-Object System.Net.Sockets.TcpClient
    $statusClient.Connect("127.0.0.1", $BackendPort)
    $statusStream = $statusClient.GetStream()
    $statusStream.ReadTimeout = 8000
    $statusHandshake = Build-Packet {
    ...
    Write-VarInt $packetBody $StatusRequestPacketId
    }
    $statusStream.Write($statusHandshake, 0, $statusHandshake.Length)
    $statusStream.Write($statusRequest, 0, $statusRequest.Length)
    $statusPacket = Read-Packet $statusStream
    if ($statusPacket.Id -ne $StatusResponsePacketId) {
    
    throw "Unexpected status packet id: $($statusPacket.Id)"
    }
    $statusJson = Read-McString $statusPacket.Stream
    $online = Extract-OnlineCount $statusJson
    if ($statusJson -notmatch '"mode":"native"') {
    
    throw "Status response does not include native marker: $statusJson"
    }
    if ($online -lt 1) {
    
    throw "Status response online count is not positive: $statusJson"
    }
    if ($statusJson -notmatch [Regex]::Escape($ExpectedUsername)) {
    
    throw "Status sample does not include expected player name: $statusJson"
    }
    Write-Host "[ONYX] E2E_STATUS_ONLINE_OK"
    Write-Host "[ONYX] STATUS_JSON=$statusJson"
    Write-Host "[ONYX] ONLINE=$online"
} finally {
    if ($statusClient -ne $null) {
    
    $statusClient.Close()
    }
    if ($loginClient -ne $null) {
