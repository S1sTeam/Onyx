param(
    ...
    [string]$ExpectedUsername = "VanillaProto",
    [string]$ExpectedMessage = "Onyx vanilla protocol bootstrap",
    [double]$ExpectedX = 18.25,
    [double]$ExpectedY = 72.0,
    [double]$ExpectedZ = -6.5,
    [float]$ExpectedYaw = 90.0,
    [float]$ExpectedPitch = 15.0,
    [string]$ExpectedWorldName = "minecraft:overworld",
    [int]$ExpectedViewDistance = 8,
    [int]$ExpectedSimulationDistance = 8,
    [int]$ExpectedTeleportId = 41
)
...
try {
    ...
    $serverConfig = Upsert-Line -content $serverConfig -pattern '(?m)^play-world-view-distance\s*=.*$' -line "play-world-view-distance = $ExpectedViewDistance"
    ...
    $hardcore = Read-Bool $packet.Stream
    ...
    $enableRespawn = Read-Bool $packet.Stream
    
    
    $limitedCrafting = Read-Bool $packet.Stream
    
    
    $worldStateDimensionId = Read-VarInt $packet.Stream
    
    
    $worldStateName = Read-McString $packet.Stream
    
    
    $hashedSeed = Read-LongBE $packet.Stream
    ...
    $isDebug = Read-Bool $packet.Stream
    
    
    $isFlat = Read-Bool $packet.Stream
    
    
    $hasDeath = Read-Bool $packet.Stream
    
    
    $portalCooldown = Read-VarInt $packet.Stream
    
    
    $seaLevel = Read-VarInt $packet.Stream
    
    
    $enforcesSecureChat = Read-Bool $packet.Stream
    ...
    if ($worldCount -ne 1 -or $worldName -ne $ExpectedWorldName) {
    
    
    
    throw "Unexpected world list in vanilla init: count=$worldCount world=$worldName"
    ...
    if ($viewDistance -ne $ExpectedViewDistance -or $simulationDistance -ne $ExpectedSimulationDistance) {
    ...
    throw "Unexpected reduced debug flag in vanilla init"
    
    
    }
    
    
    if (-not $enableRespawn) {
    
    
    
    throw "Unexpected respawn screen flag in vanilla init"
    ...
    throw "Unexpected limited crafting flag in vanilla init"
    
    
    }
    
    
    if ($worldStateDimensionId -ne 0 -or $worldStateName -ne $ExpectedWorldName) {
    
    
    
    throw "Unexpected world state in vanilla init: dim=$worldStateDimensionId world=$worldStateName"
    ...
    throw "Unexpected hashed seed: $hashedSeed"
    
    
    }
    
    
    if ($gameMode -ne 0 -or $prevGameMode -ne 255) {
    
    
    
    throw "Unexpected game mode section: current=$gameMode previous=$prevGameMode"
    
    
    }
    
    
    if ($isDebug -or $isFlat) {
    
    
    
    throw "Unexpected debug/flat flags: debug=$isDebug flat=$isFlat"
    ...
    throw "Unexpected death location flag in vanilla init"
    
    
    }
    
    
    if ($portalCooldown -ne 0) {
    
    
    
    throw "Unexpected portal cooldown: $portalCooldown"
    ...
    throw "Unexpected sea level: $seaLevel"
    ...
    throw "Unexpected secure chat enforcement flag"
    ...
    $deltaX = Read-DoubleBE $packet.Stream
    
    
    $deltaY = Read-DoubleBE $packet.Stream
    
    
    $deltaZ = Read-DoubleBE $packet.Stream
    ...
    $flags = Read-IntBE $packet.Stream
    ...
    if ([math]::Abs($deltaX) -gt 0.0001 -or [math]::Abs($deltaY) -gt 0.0001 -or [math]::Abs($deltaZ) -gt 0.0001) {
    
    
    
    throw "Unexpected spawn deltas: $deltaX $deltaY $deltaZ"
    
    
    }
    
    
    if ($flags -ne 0 -or $teleportId -ne $ExpectedTeleportId) {
    
    
    
    throw "Unexpected spawn transport fields: flags=$flags teleportId=$teleportId"
    ...
    $messageText = Read-AnonymousNbtText $packet.Stream
    
    
    $actionBar = $packet.Stream.ReadByte()
    
    
    if ($messageText -ne $ExpectedMessage) {
    
    
    
    throw "Unexpected vanilla bootstrap message text: $messageText"
    ...
    Write-Host "[ONYX] E2E_PLAY_BOOTSTRAP_PROTOCOL_VANILLA_OK"
