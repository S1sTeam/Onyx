param(
    ...
    [int]$WorldActionPacketId = 0x77,
    [int]$WorldBlockUpdatePacketId = 0x78,
    [string]$PlayProtocolMode = "onyx",
    [string]$ExpectedUsername = "WorldProbe",
...
try {
    ...
    while (($swInit.ElapsedMilliseconds -lt 5000) -and (-not ($seenWorldState -and $seenWorldChunk))) {
    ...
    throw "Did not receive world chunk packet"
    }
    ...
    if (-not $disconnectReason.Contains("world-actions=1")) {
    
    throw "Disconnect reason missing world-actions=1: $disconnectReason"
    }
    if (-not $disconnectReason.Contains("world-updates-sent=1")) {
    
    throw "Disconnect reason missing world-updates-sent=1: $disconnectReason"
    }
    Write-Host "E2E_PLAY_WORLD_OK"
} finally {
    if ($null -ne $client) {
