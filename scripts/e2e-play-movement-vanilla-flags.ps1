param(
    ...
    [int]$PositionPacketId = 0x1C,
    [int]$OnGroundPacketId = 0x1F,
    [int]$ExpectedTeleportId = 91,
    [string]$ExpectedUsername = "MoveGroundProbe",
    [double]$SpawnX = 2.5,
    [double]$SpawnY = 70.0,
    [double]$SpawnZ = -3.0,
    [double]$MoveX = 3.25,
...
try {
    ...
    throw "Unexpected packet id during movement vanilla flags run: $($packet.Id). Expected init=$BootstrapInitPacketId spawn=$BootstrapSpawnPacketId message=$BootstrapMessagePacketId disconnect=$DisconnectPacketId"
    }
    if (-not $sentConfirm) {
    
    throw "Teleport confirm packet was not sent"
    }
    if (-not $sentPosition) {
    
    throw "Position packet was not sent"
    }
    if (-not $sentOnGround) {
    
    throw "On-ground packet was not sent"
    }
    if ([string]::IsNullOrWhiteSpace($disconnectReason)) {
    
    throw "Did not receive final disconnect packet"
    }
    if ($disconnectReason -notmatch 'teleport-confirm=ok') {
    
    throw "Server did not report teleport confirm success: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'movement-packets=2') {
    
    throw "Server did not record movement packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'movement-position-packets=1') {
    
    throw "Server did not record position packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'movement-on-ground-packets=1') {
    
    throw "Server did not record on-ground packet count: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'movement-last=3\.250/70\.500/-1\.750') {
    
    throw "Server did not report expected movement coordinates: $disconnectReason"
    }
    if ($disconnectReason -notmatch 'on-ground=false') {
    
    throw "Server did not report expected on-ground state: $disconnectReason"
    }
    Write-Host "[ONYX] E2E_PLAY_MOVEMENT_VANILLA_FLAGS_OK"
