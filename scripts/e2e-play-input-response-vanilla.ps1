param(
    ...
    [int]$ChatPacketId = 0x07,
    [int]$CommandPacketId = 0x05,
    [int]$ResponsePacketId = 0x73,
    [string]$ExpectedUsername = "InputRespVan",
    [string]$ExpectedChat = "hello_onyx",
    [string]$ExpectedCommand = "ping",
    [string]$ExpectedResponseChat = "Onyx chat: hello_onyx",
    [string]$ExpectedResponseCommand = "Onyx pong"
)
...
try {
    ...
    $responseText = Read-AnonymousNbtText $packet.Stream
    
    
    $actionBarRaw = $packet.Stream.ReadByte()
    
    
    if ($actionBarRaw -lt 0) {
    
    
    throw "Unexpected EOF while reading vanilla system_chat actionBar"
    
    
    }
    
    
    if ($actionBarRaw -ne 0) {
    
    
    throw "Unexpected actionBar flag in vanilla system_chat: $actionBarRaw"
    ...
    throw "Unexpected packet id during input response vanilla run: $($packet.Id). Expected response=$ResponsePacketId or disconnect=$DisconnectPacketId"
    }
    if (-not $sentChat) {
    ...
    if ($disconnectReason -notmatch 'input-last-chat=hello_onyx') {
    ...
    Write-Host "[ONYX] E2E_PLAY_INPUT_RESPONSE_VANILLA_OK"
